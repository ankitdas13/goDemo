...
// interacting with the godemo API.
//
...
// You can also retrieve payout details as part of the response by using the expand
// parameter.
// [See docs](https://razorpay.com/docs/api/settlements/instant/fetch-with-id/) |
// [See docs with payout details](https://razorpay.com/docs/api/settlements/instant/fetch-with-id-payout)
func (r *SettlementOndemandService) Get(ctx context.Context, id string, query SettlementOndemandGetParams, opts ...option.RequestOption) (res *SettlementOndemand, err error) {
	opts = append(r.Options[:], opts...)
	if id == "" {
...
type SettlementOndemandOndemandPayouts struct {
	// Number of payout records returned.
	Count int64 `json:"count"`
	// Name of the entity. Always 'collection'.
	Entity string `json:"entity"`
	// List of payout objects for the instant settlement.
...
type SettlementOndemandOndemandPayoutsItem struct {
	// The unique identifier of the payout for the instant settlement.
	ID string `json:"id"`
	...
	CreatedAt int64 `json:"created_at"`
	// Indicates the type of entity. Here it is settlement.ondemand_payout.
	Entity string `json:"entity"`
	// Total amount (fees+tax), in paise, deducted for the payout.
	Fees int64 `json:"fees"`
	// Unix timestamp when the payout was initiated.
	...
	// Unix timestamp when the payout was reversed.
	ReversedAt int64 `json:"reversed_at,nullable"`
	// Indicates the state of the payout.
	//
	// Any of "created", "initiated", "processed", "reversed".
	Status string `json:"status"`
	// Total tax, in paise, charged for the fee component.
	Tax int64 `json:"tax"`
	// The Unique Transaction Reference (UTR) number for the payout.
...
type SettlementOndemandGetParams struct {
	// Pass this if you want to fetch payout details as part of the response for a
	// specific instant settlement. Supported values ondemand_payouts.
	//
	// Any of "ondemand_payouts".
	Expand []string `query:"expand,omitzero" json:"-"`
	paramObj
}

// URLQuery serializes [SettlementOndemandGetParams]'s query parameters as
// `url.Values`.
func (r SettlementOndemandGetParams) URLQuery() (v url.Values, err error) {
