...
// Use this endpoint to fetch the payments made on a QR Code.
// [See docs](https://razorpay.com/docs/api/qr-codes/fetch-payments)
func (r *PaymentQrCodeService) ListPayments(ctx context.Context, id string, query PaymentQrCodeListPaymentsParams, opts ...option.RequestOption) (res *PaymentQrCodeListPaymentsResponse, err error) {
	opts = append(r.Options[:], opts...)
	if id == "" {
		err = errors.New("missing required id parameter")
		return
	}
	path := fmt.Sprintf("payments/qr_codes/%s/payments", id)
	err = requestconfig.ExecuteNewRequest(ctx, http.MethodGet, path, query, &res, opts...)
	return
}
...
type PaymentQrCodeListPaymentsResponse struct {
	Count  int64     `json:"count"`
	Entity string    `json:"entity"`
...
// Returns the unmodified JSON received from the API
func (r PaymentQrCodeListPaymentsResponse) RawJSON() string { return r.JSON.raw }
func (r *PaymentQrCodeListPaymentsResponse) UnmarshalJSON(data []byte) error {
	return apijson.UnmarshalRoot(data, r)
}
...
type PaymentQrCodeListPaymentsParams struct {
	// Number of payments to be fetched. Default value is 10. Maximum value is 100.
	// This can be used for pagination, in combination with skip.
	Count param.Opt[int64] `query:"count,omitzero" json:"-"`
	// Unix timestamp, in seconds, from when payments are to be retrieved.
	From param.Opt[int64] `query:"from,omitzero" json:"-"`
	// Number of records to be skipped while fetching the payments. This can be used
	// for pagination, in combination with count.
	Skip param.Opt[int64] `query:"skip,omitzero" json:"-"`
...
// URLQuery serializes [PaymentQrCodeListPaymentsParams]'s query parameters as
// `url.Values`.
func (r PaymentQrCodeListPaymentsParams) URLQuery() (v url.Values, err error) {
	return apiquery.MarshalWithSettings(r, apiquery.QuerySettings{
		ArrayFormat:  apiquery.ArrayQueryFormatComma,
		NestedFormat: apiquery.NestedQueryFormatBrackets,
	})
